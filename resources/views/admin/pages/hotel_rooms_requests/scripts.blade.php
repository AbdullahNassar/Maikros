@section('scripts')
    ...
    <script>
        var t = $('#room_table').DataTable({
                "responsive": true,
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.getpendingroomdata') }}",
                "columns":[
                    { "data": "id" , orderable:true},
                    { "data": "r_name_en" , orderable:false},
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // Select2
            $('.select2').select2({ dropdownParent: $('#roomModal') });
            ...
            $('#add_data').click(function(){
                ...
                $('#action').val('Add');
                $('.modal-title').text('Add room Data');
                $('#imgInp').val($(this).data('imgInp'));
                $('#hotel_id').empty();
                $('#hotel_id').append('<option>             </option>');
                $.ajax({
                    url:"{{route('ajaxdata.pendinghotels')}}",
                    method:'get',
                    dataType:'json',
            ...
            $('#room_form').on('submit', function(event){
                ...
                $.ajax({
                    url:"{{ route('ajaxdata.postpendingroomdata') }}",
                    method:"POST",
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_output').html('');
                $.ajax({
                    url:"{{route('ajaxdata.fetchpendingroomdata')}}",
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
                    success:function(data)
                    {
                        ...
                        $('#imgInp').val($(this).data('imgInp'));
                        $('#r_name_en').val(data.r_name_en);
                        $('#r_name_ar').val(data.r_name_ar);
                        ...
                        $('#roomModal').modal('show');
                        $('#action').val('Edit');
                        $('.modal-title').text('Edit room Data');
                        $('#button_action').val('update');

                        if(data.image != null){
                            $('#blah').attr('src', '../storage/uploads/room/'+data.image+'');
                        ...
                        $('#hotel_id').empty();
                        $.ajax({
                            url:"{{route('ajaxdata.pendingroomhotel')}}",
                            method:'get',
                            data:{id:id},
                            dataType:'json',
                            success:function(data)
                            {
                                $.each(data, function(index, roomhotelObj){
                                    $('#hotel_id').append('<option value="'+roomhotelObj.id+'" selected>'+roomhotelObj.hotel_name_en+'</option>')
                                });
                            }
                        });
                        $.ajax({
            ...
            $(document).on('click', '.view', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#room-name').text('Hotel Room Name:  '+data.r_name_en+'');
                        if(data.image != null){
                            $('#room-image').attr('src', '../storage/uploads/room/'+data.image+'');
                        }else{
                            $('#room-image').attr('src', '../vendors/site/AR/images/hotel-detail-tab-5.jpg');
                        }
                        $('#room-content').text(''+data.content_en+'');
                    }
                });
            });
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removependingroomdata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.pendingroommassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#room_table').DataTable().ajax.reload();
