@section('scripts')
    ...
    <script>
        var t = $('#features_table').DataTable({
                ...
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.getfeaturedata') }}",
                "columns":[
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#feature_form').on('submit', function(event){
                ...
                $.ajax({
                    url:"{{ route('ajaxdata.postfeaturedata') }}",
                    method:"POST",
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_output').html('');
                $.ajax({
                    url:"{{route('ajaxdata.fetchfeaturedata')}}",
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
                    success:function(data)
                    {
                        ...
                        $('#hotel_id').empty();
                        $.ajax({
                            url:"{{route('ajaxdata.featurehotel')}}",
                            method:'get',
                            data:{id:id},
                            dataType:'json',
                            success:function(data)
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removefeaturedata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.featuremassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#features_table').DataTable().ajax.reload();
