@section('scripts')
    ...
    <script>
        var t = $('#flight_table').DataTable({
                "responsive": true,
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.getflightdata') }}",
                "columns":[
                    { "data": "id" , orderable:true},
                    { "data": "flight_name_en" , orderable:false},
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#flight_form').on('submit', function(event){
                ...
                $.ajax({
                    url:"{{ route('ajaxdata.postflightdata') }}",
                    method:"POST",
            ...
            $(document).on('click', '.edit', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.fetchflightdata')}}",
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
                    success:function(data)
                    {
                        ...
                        $('#imgInp').val($(this).data('imgInp'));
                        $('#flight_name_en').val(data.flight_name_en);
                        $('#flight_name_ar').val(data.flight_name_ar);
                        $('#duration_ar').val(data.duration_ar);
                        ...
                        //--------------------
                        if(data.image != null){
                            $('#blah').attr('src', '../storage/uploads/flight/'+data.image+'');
                        }else{
                            $('#blah').attr('src', '../vendors/site/AR/images/hotel-detail-tab-5.jpg');
                        }
                        $('#action').val('Edit');
            ...
            $(document).on('click', '.view', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#flight-name').text('Name:  '+data.flight_name_en+'');
                        $('#flight-image').attr('src', '../storage/uploads/flight/'+data.image+'');
                        $('#flight-duration').text(''+data.duration_en+'');
                        $('#flight-content').text(''+data.content_en+'');
                    }
                });
            });
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removeflightdata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.flightmassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#flight_table').DataTable().ajax.reload();
