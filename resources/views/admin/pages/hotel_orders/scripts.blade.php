@section('scripts')
    ...
    <script>
        var t = $('#order_table').DataTable({
                ...
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.gethotelOrderdata') }}",
                "columns":[
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('click', '.view', function () {
                var id = $(this).attr('id');
                $.ajax({
                    url: '{{route('ajaxdata.fetchhotelOrderdata')}}',
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
                    success: function (data) {
                        ...
                        $('#hotel-name').text('Name:  '+data.first_name+' '+data.last_name+'');
                        $('#hotel-email').text(''+data.email+'');
                        $('#hotel-phone').text(''+data.phone+'');
                        $('#hotel-country').text(''+data.country+'');
                        $('#hotel-arrival').text(''+data.arrival+'');
                        $('#hotel-leaving').text(''+data.leaving+'');
                        $('#hotel-rooms').text(''+data.rooms+'');
                        $('#hotel-beds').text(''+data.beds+'');
                        $('#hotel-adults').text(''+data.adults+'');
                        $('#hotel-children').text(''+data.children+'');
                    }
                });
                $.ajax({
                    url: '{{route('ajaxdata.orderhotel')}}',
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    success:function(data)
                    {
                        $.each(data, function(index, orderhotelObj){
                            $('#hotel-title').text('Hotel :  '+orderhotelObj.hotel_name_en+'');
                        });
                    }
                });
            });
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removehotelOrderdata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.hotelOrdermassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#hotel_table').DataTable().ajax.reload();
