@section('scripts')
    ...
    <script>
        var t = $('#member_table').DataTable({
                ...
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.getmemberdata') }}",
                "columns":[
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // Select2
            $('.select2').select2({ dropdownParent: $('#memberModal') });
            ...
            $('#add_data').click(function(){
                ...
                $('#action').val('Add');
                $('.modal-title').text('Add member Data');
            ...
            $('#member_form').on('submit', function(event){
                ...
                $.ajax({
                    url:"{{ route('ajaxdata.postmemberdata') }}",
                    method:"POST",
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_output').html('');
                $.ajax({
                    url:"{{route('ajaxdata.fetchmemberdata')}}",
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
                    success:function(data)
                    {
                        ...
                        if(data.image != null){
                            $('#blah').attr('src', '../storage/uploads/member/'+data.image+'');
            ...
            $(document).on('click', '.view', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#member-name').text('Name:  '+data.name+'');
                        if(data.image != null){
                            $('#member-image').attr('src', '../storage/uploads/member/'+data.image+'');
                        }else{
                            $('#member-image').attr('src', '../vendors/img/1.jpg');
                        }
                        $('#member-bio').text(''+data.about+'');
                        $('#member-address').text(''+data.address+'');
                        $('#member-email').text(''+data.email+'');
                        $('#member-mobile').text(''+data.phone+'');
                    }
                });
            });
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removememberdata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.membermassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#member_table').DataTable().ajax.reload();
