            ...
            $(document).on('click', '.edit', function(){
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        $('#place_id').empty();
                        $.ajax({
                            url:"{{route('ajaxdata.serviceplace')}}",
                            method:'get',
                            data:{id:id},
                            dataType:'json',
                            success:function(data)
                            {
                                $.each(data, function(index, serviceplaceObj){
                                    $('#place_id').append('<option value="'+serviceplaceObj.id+'" selected>'+serviceplaceObj.place_name_en+'</option>')
                                });
                            }
                        });
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(data.image != null){
                            ...
                            $('#service-image').attr('src', '../vendors/site/AR/images/hotel-detail-tab-5.jpg');
                        }
                        $('#service-content').text(''+data.content_en+'');
                        $('#service-price').text('Price : '+data.price+'');
                        $('#service-rate').text('Rate : '+data.rate+'');
                    }
                });
            });
