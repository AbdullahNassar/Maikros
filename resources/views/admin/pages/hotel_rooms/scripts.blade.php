    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#room_form').on('submit', function(event){
                ...
                $.ajax({
                    url:"{{ route('ajaxdata.postroomdata') }}",
                    method:"POST",
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_output').html('');
                $.ajax({
                    url:"{{route('ajaxdata.fetchroomdata')}}",
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removeroomdata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '#bulk_delete', function(){
                var id = [];
                $('.hotel_room_checkbox:checked').each(function(){
                    id.push($(this).val());
                });
                if(id.length > 0){
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.roommassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#room_table').DataTable().ajax.reload();
