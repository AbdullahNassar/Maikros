@section('scripts')
    ...
    <script>
        var t = $('#comforts_table').DataTable({
                ...
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.getcomfortdata') }}",
                "columns":[
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#comfort_form').on('submit', function(event){
                ...
                $.ajax({
                    url:"{{ route('ajaxdata.postcomfortdata') }}",
                    method:"POST",
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_output').html('');
                $.ajax({
                    url:"{{route('ajaxdata.fetchcomfortdata')}}",
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removecomfortdata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.comfortmassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#comforts_table').DataTable().ajax.reload();
