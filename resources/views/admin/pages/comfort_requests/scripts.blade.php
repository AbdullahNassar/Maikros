@section('scripts')
    ...
    <script>
        var t = $('#comforts_table').DataTable({
                "responsive": true,
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.getpendingcomfortdata') }}",
                "columns":[
                    { "data": "id" , orderable:true},
                    { "data": "c_name_en" , orderable:false},
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // Select2
            $('.select2').select2({ dropdownParent: $('#comfortModal') });
            ...
            $('#add_data').click(function(){
                ...
                $('#comfort_form')[0].reset();
                $('#form_output').html('');
                $('#button_action').val('insert');
                $('#action').val('Add');
                $('.modal-title').text('Add Comfort Data');
            ...
            $('#comfort_form').on('submit', function(event){
                event.preventDefault();
                //var form_data = $(this).serialize();
                var action = $('#button_action').val();
                $.ajax({
                    url:"{{ route('ajaxdata.postpendingcomfortdata') }}",
                    method:"POST",
                    ...
                    success:function(data)
                    {
                        ...
                        else
                        {
                            if (action == 'insert') {
                                ...
                                $('#comfort_form')[0].reset();
                                $('#action').val('Add');
                                $('.modal-title').text('Add New Comfort');
                                $('#button_action').val('insert');
                                $('#comforts_table').DataTable().ajax.reload();
                                $('#blah').attr('src', '../vendors/site/AR/images/hotel-detail-tab-5.jpg');
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_output').html('');
                $.ajax({
                    url:"{{route('ajaxdata.fetchpendingcomfortdata')}}",
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
                    success:function(data)
                    {
                        ...
                        $('#imgInp').val($(this).data('imgInp'));
                        $('#name_en').val(data.c_name_en);
                        $('#name_ar').val(data.c_name_ar);
                        $('#hotel_id').val(data.hotel_id);
                        $('#comfort_id').val(id);
                        $('#comfortModal').modal('show');
                        $('#action').val('Edit');
                        $('.modal-title').text('Edit Comfort Data');
                        $('#button_action').val('update');

                        if(data.image != null){
                            $('#blah').attr('src', '../storage/uploads/comfort/'+data.image+'');
                        ...
                        $('#hotel_id').empty();
                        $.ajax({
                            url:"{{route('ajaxdata.pendingcomforthotel')}}",
                            method:'get',
                            data:{id:id},
                            dataType:'json',
                            success:function(data)
                            {
                                $.each(data, function(index, comforthotelObj){
                                    $('#hotel_id').append('<option value="'+comforthotelObj.id+'" selected>'+comforthotelObj.hotel_name_en+'</option>')
                                });
                            }
                        });
                        $.ajax({
            ...
            $(document).on('click', '.view', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#comfort-name').text('Hotel Comfort Name:  '+data.c_name_en+'');
                        if(data.image != null){
                            $('#comfort-image').attr('src', '../storage/uploads/comfort/'+data.image+'');
                        }else{
                            $('#comfort-image').attr('src', '../vendors/site/AR/images/hotel-detail-tab-5.jpg');
                        }
                    }
                });
            });
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removependingcomfortdata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '#bulk_delete', function(){
                var id = [];
                $('.comfort_checkbox:checked').each(function(){
                    id.push($(this).val());
                });
                if(id.length > 0){
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.pendingcomfortmassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#comforts_table').DataTable().ajax.reload();
