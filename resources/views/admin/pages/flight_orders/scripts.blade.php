@section('scripts')
    ...
    <script>
        var t = $('#order_table').DataTable({
                ...
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.getflightOrderdata') }}",
                "columns":[
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('click', '.view', function () {
                var id = $(this).attr('id');
                $.ajax({
                    url: '{{route('ajaxdata.fetchflightOrderdata')}}',
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
                    success: function (data) {
                        ...
                        $('#flight-name').text('Name:  '+data.first_name+' '+data.last_name+'');
                        $('#flight-email').text(''+data.email+'');
                        $('#flight-phone').text(''+data.phone+'');
                        $('#flight-country').text(''+data.country+'');
                        $('#hotel-destination').text(''+data.destination+'');
                        $('#hotel-arrival').text(''+data.arrival+'');
                        $('#hotel-leaving').text(''+data.leaving+'');
                        $('#hotel-status').text(''+data.status+'');
                        $('#hotel-class').text(''+data.class+'');
                        $('#hotel-adults').text(''+data.adults+'');
                        $('#hotel-children').text(''+data.children+'');
                    }
                });
                $.ajax({
                    url: '{{route('ajaxdata.orderflight')}}',
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    success:function(data)
                    {
                        $.each(data, function(index, orderflightObj){
                            $('#flight-title').text('flight :  '+orderflightObj.flight_name_en+'');
                        });
                    }
                });
            });
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removeflightOrderdata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
                        $('#flight_table').DataTable().ajax.reload();
                    }
                })
            ...
            $(document).on('click', '#bulk_delete', function(){
                var id = [];
                $('.flight_checkbox:checked').each(function(){
                    id.push($(this).val());
                });
                if(id.length > 0){
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.flightOrdermassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#flight_table').DataTable().ajax.reload();
