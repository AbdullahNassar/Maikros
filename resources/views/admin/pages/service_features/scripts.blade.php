@section('scripts')
    ...
    <script>
        var t = $('#features_table').DataTable({
                ...
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('ajaxdata.getservicefeaturedata') }}",
                "columns":[
    ...
    <script type="text/javascript">
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        $.each(data, function(index, serviceObj){
                            $('#service_id').append('<option value="'+serviceObj.id+'">'+serviceObj.name_en+'</option>')
                        });
                    }
                });
            });
            ...
            $('#feature_form').on('submit', function(event){
                ...
                $.ajax({
                    url:"{{ route('ajaxdata.postservicefeaturedata') }}",
                    method:"POST",
            ...
            $(document).on('click', '.edit', function(){
                ...
                $('#form_output').html('');
                $.ajax({
                    url:"{{route('ajaxdata.fetchservicefeaturedata')}}",
                    method:'get',
                    data:{id:id},
                    dataType:'json',
                    cache: false,
                    success:function(data)
                    {
                        ...
                        $('#service_id').empty();
                        $.ajax({
                            url:"{{route('ajaxdata.featureservice')}}",
                            method:'get',
                            data:{id:id},
                            dataType:'json',
                            success:function(data)
                            {
                                $.each(data, function(index, featureserviceObj){
                                    $('#service_id').append('<option value="'+featureserviceObj.id+'" selected>'+featureserviceObj.name_en+'</option>')
                                });
                            }
                        });
                        $.ajax({
            ...
            $(document).on('click', '.view', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#feature-name').text('service Feature Name:  '+data.f_name_en+'');
                        if(data.image != null){
            ...
            $(document).on('click', '.btndelete', function(){
                ...
                $.ajax({
                    url:"{{route('ajaxdata.removeservicefeaturedata')}}",
                    mehtod:"get",
                    data:{id:delete_id},
                    success:function(data)
                    {
            ...
            $(document).on('click', '.multideletebtn', function(){
                ...
                if(id.length > 0){
                    $.ajax({
                        url:"{{ route('ajaxdata.servicefeaturemassremove')}}",
                        method:"get",
                        data:{id:id},
                        success:function(data){
                            $('#features_table').DataTable().ajax.reload();
