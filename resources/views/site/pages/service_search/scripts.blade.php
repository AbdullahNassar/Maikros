...
<script src="{{asset('vendors/site/EN/js/custom-price-slider.js')}}"></script>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#add_cart{{$loop->index}}').on('submit', function(event){
            event.preventDefault();
            $.ajax({
                url:"{{ route('site.addServiceCart') }}",
                method:"POST",
                ...
                success:function(data)
                {
                    ...
                    {
                        toastr.info('Booking Service is Done Successfully.', 'Success!', {timeOut: 3000});
                        $('#add_cart')[0].reset();                            
                    }
                }
            })
        ...
        $('#add_cart{{$loop->index}}').on('submit', function(event){
            ...
            $.ajax({
                ...
                success:function(data)
                {
                    ...
                    {
                        toastr.success('تم حجز الخدمة بنجاح', 'تم!', {timeOut: 3000});
                        $('#add_cart')[0].reset();
