...
class PendingHotelsController extends Controller {

    public function getIndex() {
        return view('admin.pages.hotel_requests.index');
    }

    public function gallery($id) {
        $hotel = Hotel::find($id);
        $gallery = HotelGallery::get()->where('hotel_id','=',$id);
        return view('admin.pages.hotel_requests.gallery', compact('gallery','hotel'));
    }

    public function getdata() {
        $hotels = Hotel::select('id','hotel_name_en','price')->where('active','=', -1)->get();
